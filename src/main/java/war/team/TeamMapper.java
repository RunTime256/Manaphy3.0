...
public interface TeamMapper
{
    @Select("WITH team_tokens AS (SELECT t.short_name, COALESCE(SUM(p.value), 0) AS tokens FROM cc4.team t LEFT JOIN cc4.member m ON t.id = m.team_id " +
            ...
            "SELECT t.role_id, t.short_name, 'test' AS full_name, t.welcome_text, t.leader_image, t.color as color, count(m.user_id) AS member_count, tt.tokens AS prewar_tokens " +
            ...
            "GROUP BY t.role_id, t.short_name, t.welcome_text, t.leader_image, t.color, tt.tokens")
    @Results(value = {
