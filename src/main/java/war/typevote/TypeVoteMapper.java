...
public interface TypeVoteMapper
{
    ...
    @Select("SELECT EXISTS(SELECT * FROM cc4.puzzle p LEFT JOIN cc4.puzzle_solution ps ON p.id = ps.puzzle_id " +
            ...
            "LEFT JOIN cc4.type_vote_selection tvs ON tv.id = tvs.type_id AND tvs.user_id = #{userId} " +
            "WHERE pg.user_id = #{userId} AND tv.type IS NOT NULL AND tvs.id IS NULL AND tv.type = #{type} AND pg.guess = ps.solution)")
    ...
    @Select("SELECT DISTINCT tv.type FROM cc4.puzzle p LEFT JOIN cc4.puzzle_solution ps ON p.id = ps.puzzle_id " +
            ...
            "WHERE pg.user_id = #{userId} AND tvs.user_id = #{userId} AND tv.type IS NOT NULL AND tvs.id IS NOT NULL")
    ...
    @Select("SELECT count(DISTINCT tv.type) FROM cc4.puzzle p LEFT JOIN cc4.puzzle_solution ps ON p.id = ps.puzzle_id " +
            ...
            "LEFT JOIN cc4.type_vote_selection tvs ON tv.id = tvs.type_id " +
            "WHERE pg.user_id = #{userId} AND tvs.user_id = #{userId} AND tv.type IS NOT NULL AND tvs.id IS NOT NULL")
    int getUsedVoteCount(@Param("userId") long userId);
