...
public interface LeaderboardMapper
{
    @Select("WITH winner AS (SELECT y.id, COALESCE(y.tokens, 0) AS tokens FROM " +
            ...
            "LEFT JOIN cc4.team t ON t.id = m.team_id WHERE b.timestamp > '2020-12-21 05:00:00' " +
            ...
            "LEFT JOIN cc4.hoard_hunt hh ON hh.user_id = m.user_id AND hh.timestamp > '2020-12-21 05:00:00' " +
            "WHERE m.team_id IS NOT NULL) y " +
            "ON 1 = 1 GROUP BY y.id) " +
            "SELECT t.full_name AS team, t.color, COALESCE((w.tokens + l.tokens + h.tokens), 0) AS tokens " +
            "FROM cc4.team t LEFT JOIN winner w ON w.id = t.id LEFT JOIN loser l ON w.id = l.id " +
    ...
    @Select("WITH solved AS (SELECT DISTINCT p.id, pg.user_id, p.value AS tokens FROM cc4.puzzle p " +
            ...
            "WHERE p.prewar = false AND p.end_time IS NOT NULL AND p.end_time > '2020-12-21 05:00:00'), " +
    ...
    @Select("WITH fp AS (SELECT m.team_id, SUM(fp.tokens) AS tokens FROM cc4.fanart_participation fp " +
            "LEFT JOIN cc4.member m ON fp.user_id = m.user_id WHERE fp.timestamp > '2020-12-21 05:00:00' GROUP BY m.team_id), " +
            "fb AS (SELECT m.team_id, SUM(fb.tokens) AS tokens FROM cc4.fanart_bonus fb " +
            "LEFT JOIN cc4.member m ON fb.user_id = m.user_id WHERE fb.timestamp > '2020-12-21 05:00:00' GROUP BY m.team_id), " +
            ...
            "LEFT JOIN cc4.member m ON cp.user_id = m.user_id WHERE c.end_time > '2020-12-21 05:00:00' GROUP BY m.team_id), " +
            ...
            "LEFT JOIN cc4.member m ON cw.user_id = m.user_id WHERE c.end_time > '2020-12-21 05:00:00' GROUP BY m.team_id) " +
    ...
    @Select("SELECT t.full_name AS team, t.color AS color, COALESCE(SUM(gp.tokens), 0) AS tokens " +
            ...
            "WHERE gp.timestamp > '2020-12-21 05:00:00' AND gp.game_id != 1 " +
            "GROUP BY t.full_name, t.color ORDER BY tokens DESC")
    @Results(value = {
            @Result(property = "teamName", column = "team"),
            @Result(property = "colorValue", column = "color"),
    ...
    @Select("SELECT t.full_name AS team, t.color AS color, COALESCE(SUM(adt.tokens), 0) AS tokens " +
            ...
            "WHERE a.prewar = false AND ua.timestamp > '2020-12-21 05:00:00' " +
            "GROUP BY t.full_name, t.color ORDER BY tokens DESC")
    @Results(value = {
            @Result(property = "teamName", column = "team"),
            @Result(property = "colorValue", column = "color"),
